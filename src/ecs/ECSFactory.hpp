...
public:
    ...
    ECSFactory() : m_chaiDefModule(std::make_shared<chaiscript::Module>()),
                   m_chaiCreateModule(std::make_shared<chaiscript::Module>())
    {
        m_chaiDefModule->add(chaiscript::fun(&ECSFactory::defineObject),
                             "defineObject");
    ...
    */
    ...
    bool registerComponent(const std::string &name)
    {
        ...
        m_chaiDefModule->add(chaiscript::fun(&ComponentCascading::addComponent<TComp>),
                             "addComp_" + name);

        if constexpr (sizeof...(TArgs) != 0) {
            m_chaiDefModule->add(chaiscript::fun(&ComponentCascading::addComponent<TComp, TArgs...>),
                                 "addComp_" + name);
        }
