...
Texture2D::Texture2D(const std::string& filepath,
  ...
  m_border(border)
{
  m_gl_texID = m_texManager.loadTexture2D(filepath, m_width, m_height, m_numChannels, internal_format, image_format, level, border);
...
void Texture2D::makeDeepCopy(const Texture2D& tex) {
  if (tex.m_filepath.has_value()) {
    m_gl_texID = m_texManager.loadTexture2D(tex.m_filepath.value(), m_width, m_height, m_numChannels, tex.m_internal_format, tex.m_image_format, tex.m_level, tex.m_border);
    ...
    m_gl_texID = m_texManager.createTexture2D(tex.m_width, tex.m_height, tex.m_internal_format, tex.m_image_format, tex.m_level, tex.m_border);
    if (m_gl_texID == 0) throw std::runtime_error("ERROR: Could not create 2D texture!");
    m_width = tex.m_width;
...
Texture2D::~Texture2D() {
  std::cout << "Start delete Texture2D " << m_gl_texID  << std::endl;
  if(m_gl_texID != 0) m_texManager.deleteTexture(m_gl_texID);
  std::cout << "End delete Texture2d " << m_gl_texID << std::endl;
}